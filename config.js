...
/* V86.45.11 — question-admin is leader-only; opens inside Smart Leader Center. */
(function(){
...
(function(){
  ...
  function loadLeaderAdmin(){load('training-question-admin.js?v=86.45.11')}
  function loadQuestionAdminForLeader(){
    ...
    /* login already stores the authoritative role; load the leader editor immediately. */
    if(cachedRole==='leader')loadLeaderAdmin();
    fetch('/api/rpc',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({fn:'training_get_session',args:{p_token:token}}),cache:'no-store'})
      ...
      .then(function(d){
        ...
        if(role==='leader')loadLeaderAdmin();
        else if(role)removeMemberAdmin();
      })
