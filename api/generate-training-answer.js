...
module.exports=async function handler(req,res){
  ...
  try{
    ...
    const ai=await fetch('https://api.openai.com/v1/responses',{method:'POST',headers:{Authorization:`Bearer ${OPENAI_API_KEY}`,'Content-Type':'application/json'},body:JSON.stringify({
      model:OPENAI_MODEL,
      instructions:[
    ...
    const text=await ai.text();let data=null;try{data=text?JSON.parse(text):null}catch(_){data=null}
    if(!ai.ok)return json(res,502,{error:(data&&data.error&&data.error.message)||text||'فشل توليد الجواب المثالي'});
    const out=outputText(data);if(!out)return json(res,502,{error:'لم يرجع الذكاء الاصطناعي جوابًا'});
    let parsed;try{parsed=JSON.parse(out)}catch(_){return json(res,502,{error:'نتيجة التوليد غير صالحة'});}
    const ideal=String(parsed&&parsed.model_answer||'').trim();if(!ideal)return json(res,502,{error:'الجواب المثالي فارغ'});
