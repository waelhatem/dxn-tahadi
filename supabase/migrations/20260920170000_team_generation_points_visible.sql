-- Team generations: return full member fields plus explicit cumulative_points.
-- This keeps PV visible and exposes the accumulated/group points needed by the UI.
...
begin
  ...
  if mode_name='summary' then
    ...
    with recursive tree as (
      select d.*,1 as depth_from_target
      from public.dxn_team_members d
      where d.sponsor_member_no=target_no

      union all

      select d.*,t.depth_from_target+1
      from public.dxn_team_members d
      join tree t on d.sponsor_member_no=t.member_no
    ...
    select jsonb_build_object(
      ...
      'members',coalesce((
        select jsonb_agg(to_jsonb(x) order by x.depth_from_target,x.member_name nulls last,x.member_no)
      ...
      coalesce((
        select jsonb_agg(
          to_jsonb(x) || jsonb_build_object(
            'cumulative_points',
            coalesce(x.total_group_pv,x.personal_group_pv)
          )
          order by
            coalesce(x.total_group_pv,x.personal_group_pv,0) desc,
            coalesce(x.personal_pv,0) desc,
            x.member_name nulls last,
            x.member_no
        )
        from (
          select *
