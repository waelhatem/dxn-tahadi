-- Enforce the team-data visibility rule at the database boundary.
-- Every authenticated account may read only itself + its own recursive downline.
-- The root member number is derived from the authenticated session, never supplied by the caller.

drop function if exists public.get_dxn_team_intelligence(text,text,text,integer,integer);
...
begin
  ...
  -- Any requested target must be the authenticated member or a member
  -- recursively sponsored below that member.
  ...
  if mode_name = 'summary' then
    with recursive tree as (
      select m.member_no, m.generation, m.rank, m.personal_pv, m.personal_group_pv,
             m.total_group_pv, 0 as depth
      from public.dxn_team_members m
      where m.member_no = root_no

      union all

      select d.member_no, d.generation, d.rank, d.personal_pv, d.personal_group_pv,
             d.total_group_pv, t.depth + 1
      from public.dxn_team_members d
      join tree t on d.sponsor_member_no = t.member_no
      where t.depth < 20
    ),
    by_generation as (
      select generation, count(*) as members
      from tree
      group by generation
      order by generation
    ),
    by_rank as (
      select coalesce(nullif(trim(rank), ''), 'غير محدد') as rank, count(*) as members
      from tree
      group by coalesce(nullif(trim(rank), ''), 'غير محدد')
    ...
    with recursive tree as (
      select d.member_no, d.member_name, d.sponsor_member_no, d.sponsor_name,
             d.generation, d.rank, d.dxn_status, d.downline_status, d.join_date,
             d.personal_pv, d.personal_group_pv, d.total_group_pv,
    ...
    if p_generation is null or p_generation < 0 then
      raise exception using errcode='P0001', message='حدد رقم الجيل المطلوب';
    end if;

    with recursive tree as (
      select m.member_no, m.generation, 0 as depth
      from public.dxn_team_members m
      where m.member_no = target_no
    ...
    select jsonb_build_object(
      ...
      'count',(select count(*) from tree t where t.generation = p_generation),
      'members',coalesce((
        select jsonb_agg(to_jsonb(m) order by m.member_name nulls last, m.member_no)
        from (
          select d.*
          from public.dxn_team_members d
          join tree t on t.member_no = d.member_no
  ...
  elsif mode_name = 'line_summary' then
    with recursive lines as (
      select d.member_no, d.member_name, d.generation, d.rank,
             d.personal_pv, d.total_group_pv
      from public.dxn_team_members d
      where d.sponsor_member_no = root_no
    ),
    line_tree as (
      select l.member_no as line_root, l.member_no, l.personal_pv, l.total_group_pv, 0 as depth
      from lines l
      ...
      from line_tree lt
      join public.dxn_team_members d on d.sponsor_member_no = lt.member_no
      where lt.depth < 20
    ),
    stats as (
      select line_root,
             count(*) as members,
             coalesce(sum(personal_pv),0) as personal_pv_total,
             coalesce(sum(total_group_pv),0) as total_group_pv_sum
      from line_tree
      group by line_root
    )
    select jsonb_build_object(
      ...
      'lines',coalesce((
        select jsonb_agg(
          jsonb_build_object(
            'line_member_no',l.member_no,
            'line_member_name',l.member_name,
            'line_generation',l.generation,
            'line_rank',l.rank,
            'members',s.members,
            'personal_pv_total',s.personal_pv_total,
            'total_group_pv_sum',s.total_group_pv_sum
          )
          order by s.members desc, s.personal_pv_total desc, l.member_name nulls last
        )
        from lines l
        join stats s on s.line_root = l.member_no
      ),'[]'::jsonb)
    ) into result;
  end if;
...
grant execute on function public.get_dxn_team_intelligence(uuid,text,text,integer,integer)
  to service_role;
