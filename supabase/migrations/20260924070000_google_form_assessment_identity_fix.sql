-- Normalize Google Form assessment identity lookups.
-- Sponsor identity remains tied to Form ID in Apps Script.
-- Membership number remains the NEW MEMBER's number entered in Google Form.
-- This migration only normalizes whitespace/hyphens during lookups.
...
declare
  ...
  target_no text := nullif(
    regexp_replace(
      trim(coalesce(p_membership_number, '')),
      '[[:space:]-]+',
      '',
      'g'
    ),
...
begin
  ...
  where regexp_replace(
    trim(coalesce(e.membership_number, '')),
    '[[:space:]-]+',
    '',
    'g'
  ) = target_no;
