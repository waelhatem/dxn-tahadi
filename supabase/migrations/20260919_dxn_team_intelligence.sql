-- DXN Team Intelligence read-only RPC.
-- The agent supplies the authenticated member as p_root_member_no.
-- Target members are restricted to that member's own downline.
...
begin
  if root_no = '' then
    raise exception 'رقم العضوية الأساسي مطلوب';
  end if;

  if not exists (
    select 1 from public.dxn_team_members m where m.member_no = root_no
  ) then
    raise exception 'العضو الأساسي غير موجود في سجل DXN';
  end if;
  ...
  -- Build the authenticated member's full reachable tree.
...
revoke all on function public.get_dxn_team_intelligence(text,text,text,integer,integer)
  from public, anon, authenticated;

grant execute on function public.get_dxn_team_intelligence(text,text,text,integer,integer)
  to service_role;
