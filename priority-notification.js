...
(function(){
  ...
  function getAppData(){
    try{
      if(typeof data!=='undefined' && data && Array.isArray(data.challenges)) return data;
    }catch(e){}
    return null;
  }
  function getRole(){
    try{if(typeof role!=='undefined') return role}catch(e){}
    return null;
  }
  ...
  function chooseChallenge(d){
    ...
    var pending=list.filter(function(c){
      return !subs.some(function(s){return String(s.challenge_id||s.challengeId||'')===String(c.id) && isToday(s.created_at||s.submitted_at||s.updated_at) && String(s.status||'').toLowerCase()!=='rejected'})
    });
  ...
  function show(c){
    ...
    var stars=Number(c.stars||c.reward_stars||0);
    var title=esc(c.title||'التحدي الأهم اليوم');
    var desc=esc(c.description||'هذا هو التحدي الذي نريد منك أن تبدأ به الآن وتنجزه بأسرع وقت ممكن.');
    ...
    box.querySelector('.dxn-priority-close').onclick=close;
    box.querySelector('.dxn-priority-later').onclick=close;
    box.querySelector('.dxn-priority-backdrop').onclick=close;
    box.querySelector('.dxn-priority-start').onclick=function(){close();try{if(typeof switchTab==='function')switchTab('ch')}catch(e){}};
  }
  function run(){
    if(getRole() && String(getRole()).toLowerCase()!=='member')return;
    if(sessionStorage.getItem(shownKey))return;
    var d=getAppData();if(!d)return;
