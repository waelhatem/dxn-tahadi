...
(function(){
  ...
  function renderMember(force){
    ...
    var root=document.getElementById('dxn-training-assessment');if(root&&!force)return;
    if(!root){root=document.createElement('section');root.id='dxn-training-assessment';root.className='card';root.style.cssText='margin:18px 0;border:2px solid #cfe4da;background:linear-gradient(135deg,#f8fcfa,#fff);direction:rtl;text-align:right';document.body.appendChild(root)}
  ...
  async function load(force){var t=token();if(!t)return;try{var d=await rpc('training_assessment_bootstrap',{p_token:t});state=d||state;state.loaded=true;renderMember(!!force);renderLeader(!!force)}catch(e){console.error('training assessment',e)}}
  window.submitTrainingAnswer=async function(qid){var el=document.querySelector('[data-assessment-q="'+String(qid).replace(/"/g,'\\"')+'"]');var value=el?String(el.value||'').trim():'';if(value.length<2){alert('اكتب إجابة قبل الإرسال.');return}try{await rpc('submit_training_answer',{p_token:token(),p_question_id:qid,p_answer:value,p_attempt_no:1});await load(true);alert('تم إرسال الإجابة للمراجعة.')}catch(e){alert('تعذر إرسال الإجابة: '+e.message)}};
  window.reviewTrainingAnswer=async function(id,status){var s=document.getElementById('score-'+id),n=document.getElementById('note-'+id);var score=Math.max(0,Math.min(100,Number(s&&s.value||0)));var note=String(n&&n.value||'');try{await rpc('review_training_answer',{p_token:token(),p_answer_id:id,p_status:status,p_score:score,p_note:note});await load(true);}catch(e){alert('تعذر حفظ المراجعة: '+e.message)}};
  function start(){var tries=0,t=setInterval(function(){if(token()){load(true);clearInterval(t)}else if(++tries>300)clearInterval(t)},100);var mo=new MutationObserver(function(){if(state.loaded){if(state.role==='member')renderMember(false);if(state.role==='leader')renderLeader(false)}});mo.observe(document.body,{childList:true,subtree:true})}
