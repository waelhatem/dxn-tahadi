/* V86.44.5 — زر اختبار التدريب تحت زر المشاهدة/البدء مع قفل مطابق لفتح التدريب */
(function(){
  if(window.__DXN_TRAINING_ASSESSMENT_BUTTONS_V86445__) return;
  window.__DXN_TRAINING_ASSESSMENT_BUTTONS_V86445__=true;

  function esc(v){return String(v==null?'':v).replace(/[&<>\"']/g,function(c){return {'&':'&amp;','<':'&lt;','>':'&gt;','\"':'&quot;',"'":'&#39;'}[c]})}
  function isMember(){return String(window.role||window.currentRole||'').toLowerCase()==='member' || !!document.querySelector('[data-role="member"]')}
  function lessons(){
  ...
  function findCard(lesson,index){
    var id=videoId(lesson.video_url||lesson.url||lesson.video||'');
    var nodes=ytNodes();
    if(id){
  ...
  function isLocked(card,action){
    ...
    if(/locked|lock|disabled/i.test(cls))return true;
    return false;
  }
  function findAssessment(no){
    ...
    for(var i=0;i<ds.length;i++){
      var t=String(ds[i].textContent||'');
      if(new RegExp('التدريب\\s*'+Number(no)+'(?:\\D|:)').test(t))return ds[i];
    }
  ...
  function addButton(card,lesson,action){
    ...
    var no=Number(lesson.lesson_no),locked=isLocked(card,action);
    ...
    b.disabled=locked;
    b.title=locked?'يُفتح الاختبار عند فتح التدريب بالتقدم.':'الانتقال إلى اختبار هذا التدريب';
    ...
    if(action&&action.parentNode){
      action.parentNode.insertBefore(b,action.nextSibling);
    }else{
      var actions=card.querySelector('.actions');
      if(actions)actions.appendChild(b);else card.appendChild(b);
    }
  }
  function refresh(){
    if(!isMember())return;
    ...
    ls.forEach(function(l,i){
      ...
      var acts=actionButtons(card),action=acts[0]||null;
      addButton(card,l,action);
      var b=card.querySelector('[data-training-assessment-button="'+Number(l.lesson_no)+'"]');
      if(!b)return;
      var locked=isLocked(card,action);
      b.disabled=locked;
      b.style.background=locked?'#eef1f1':'#eef8f2';
      b.style.color=locked?'#7b8790':'#0f513f';
      b.style.borderColor=locked?'#d8dee0':'#cfe4da';
      b.innerHTML=locked?'🔒 اختبار التدريب — مغلق حتى يفتح التدريب':'🧠 اختبار التدريب';
      b.title=locked?'يُفتح الاختبار عند فتح التدريب بالتقدم.':'الانتقال إلى اختبار هذا التدريب';
  ...
  function boot(){
    refresh();
    if(++tries<80)setTimeout(boot,500);
  }
