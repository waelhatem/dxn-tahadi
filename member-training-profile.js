...
(function(){
  ...
  function pctOf(p){return Math.max(0,Math.min(100,Number(p&&p.completed===true?100:(p&&p.watch_percent||0))));}
  ...
  function detailedMemberProfile(id){
    try{
      var appData=(typeof data!=='undefined'&&data)?data:{};
      var members=Array.isArray(appData.members)?appData.members:[];
      var m=members.find(function(x){return String(x.id)===String(id)});
      if(!m){if(typeof toast==='function')toast('تعذر العثور على بيانات العضو.');return;}
      var snap=(typeof memberActivitySnapshot==='function'?memberActivitySnapshot().find(function(x){return String(x.id)===String(id)}):null)||m;
      var submissions=Array.isArray(appData.my_submissions)?appData.my_submissions:[];
      ...
      var td=(typeof trainingData!=='undefined'&&trainingData)?trainingData:{};
      ...
      var pts=Number(snap._pts||0),target=Math.max(0,100-pts),badgeCount=0;
      try{if(typeof getBadges==='function'){var oldMe=me;me=m;badgeCount=(getBadges()||[]).filter(function(b){return b.earned}).length;me=oldMe;}}catch(e){}
      ...
      var lessonRows=lessons.map(function(l){
        ...
        var updated=p&&(p.updated_at||p.last_update_at||p.completed_at),updatedText=updated?' · 🕐 '+new Date(updated).toLocaleString('ar-IQ'):'';
      ...
      var level='';try{if(typeof getLevel==='function')level=getLevel(m.stars).current.name||''}catch(e){}
      var rating='';try{if(typeof memberRatingCard==='function')rating=memberRatingCard(id,typeof role!=='undefined'?role:'')}catch(e){}
    ...
    }catch(e){console.error('V86.13 member profile',e);if(typeof toast==='function')toast('تعذر فتح الملف التفصيلي: '+(e&&e.message?e.message:'خطأ غير معروف'));}
  }
  window.openMemberProfile=detailedMemberProfile;
  window.__DXNOpenDetailedMemberProfile=detailedMemberProfile;
  function wireButtons(){
    try{document.querySelectorAll('button').forEach(function(btn){var text=(btn.textContent||'').trim();if(!text.includes('الملف التفصيلي')||btn.__dxnProfileV8613)return;btn.__dxnProfileV8613=true;var attr=btn.getAttribute('onclick')||'',match=attr.match(/openMemberProfile\s*\(\s*[\'\"]?([^\'\")\s]+)[\'\"]?\s*\)/),id=match?match[1]:btn.dataset.memberId||'';btn.dataset.memberProfileId=id;btn.addEventListener('click',function(e){e.preventDefault();e.stopImmediatePropagation();var x=btn.dataset.memberProfileId;if(x)detailedMemberProfile(x);else if(typeof toast==='function')toast('تعذر تحديد عضو الملف التفصيلي.');},true);});}catch(e){console.debug('V86.13 profile button wiring',e)}
  }
  window.__DXNWireMemberProfileButtons=wireButtons;wireButtons();
